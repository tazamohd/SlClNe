...
export const moduleId = z.enum([
   ...
   * apart from ordinary `ai` use so a Branch Manager or Service Advisor who
   ...
   * `admin` (tenant-scoped departments/branches/users) and `settings`
   * (tenant-scoped config) so a tenant role granted either of those never
   * inherits this one. */
  'superadmin',
  'audit',
  'network',
...
export const FIELD_RULES: readonly { field: string; ar: string; hidden: readonly RoleId[] }[] = [
  ...
  /* DVHC (Sprint 2, P0). RLS lets a customer read their own job's inspection
   * findings — the point of a customer-facing health-check report — but a
   * technician's internal note is a column those rows carry, not a row a
   * policy could deny outright. `server/src/registry.ts`'s `REDACTIONS` nulls
   * it for whatever role this rule hides it from. */
