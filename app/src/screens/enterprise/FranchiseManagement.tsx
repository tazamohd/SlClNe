 ...
 *  `franchiseManagement` once the API serves it. */
export function FranchiseManagement() {
  const { t } = usePreferences()
  const isMobile = useIsMobile()
  const gap = <GapCard icon="Building2" collection="franchiseManagement" />

  if (isMobile) {
    return (
      <div className="flex animate-fade-up flex-col gap-4 motion-reduce:animate-none">
        <MobilePageHeader icon="Building2" title={t('Franchise Management')} subtitle={t('Manage franchise locations and operators')} />
        {gap}
      </div>
    )
  }
