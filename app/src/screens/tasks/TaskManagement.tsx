 ...
 *  `taskManagement` once the API serves it. */
export function TaskManagement() {
  const { t } = usePreferences()
  const isMobile = useIsMobile()
  const gap = <GapCard icon="ListTodo" collection="taskManagement" />
  ...
  return (
    <div className="flex animate-fade-up flex-col gap-6 motion-reduce:animate-none">
      <PageHeader icon="ListTodo" title={t('Task Board')} subtitle={t('Manage tasks')} />
      {gap}
    </div>
  )
}
