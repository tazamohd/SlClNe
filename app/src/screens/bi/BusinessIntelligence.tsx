 ...
 *  `businessIntelligence` once the API serves it. */
export function BusinessIntelligence() {
  const { t } = usePreferences()
  const isMobile = useIsMobile()
  const gap = <GapCard icon="BarChart3" collection="businessIntelligence" />

  if (isMobile) {
    return (
      <div className="flex animate-fade-up flex-col gap-4 motion-reduce:animate-none">
        <MobilePageHeader icon="BarChart3" title={t('Business Intelligence')} subtitle={t('Analytics overview and module status')} />
        {gap}
      </div>
    )
  }
