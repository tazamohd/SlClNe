 ...
 *  `businessHeatmaps` once the API serves it. */
export function BusinessHeatmaps() {
  const { t } = usePreferences()
  const isMobile = useIsMobile()
  const gap = <GapCard icon="Grid3x3" collection="businessHeatmaps" />
  ...
  return (
    <div className="flex animate-fade-up flex-col gap-6 motion-reduce:animate-none">
      <PageHeader icon="Grid3x3" title={t('Heatmaps')} subtitle={t('Business Intelligence')} />
      {gap}
    </div>
  )
}
