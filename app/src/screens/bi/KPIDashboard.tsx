 ...
 *  `kPIDashboard` once the API serves it. */
export function KPIDashboard() {
  const { t } = usePreferences()
  const isMobile = useIsMobile()
  const gap = <GapCard icon="Target" collection="kPIDashboard" />

  if (isMobile) {
    return (
      <div className="flex animate-fade-up flex-col gap-4 motion-reduce:animate-none">
        <MobilePageHeader icon="Target" title={t('KPI Dashboard')} subtitle={t('Key performance indicators at a glance')} />
        {gap}
      </div>
    )
  }
