 ...
 *  `documentManagement` once the API serves it. */
export function DocumentManagement() {
  const { t } = usePreferences()
  const isMobile = useIsMobile()
  const gap = <GapCard icon="FileText" collection="documentManagement" />
  ...
  return (
    <div className="flex animate-fade-up flex-col gap-6 motion-reduce:animate-none">
      <PageHeader icon="FileText" title={t('Documents')} subtitle={t('Document management')} />
      {gap}
    </div>
  )
}
