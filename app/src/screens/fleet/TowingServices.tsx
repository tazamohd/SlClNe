 ...
 *  `towingServices` once the API serves it. */
export function TowingServices() {
  const { t } = usePreferences()
  const isMobile = useIsMobile()
  const gap = <GapCard icon="Truck" collection="towingServices" />

  if (isMobile) {
    return (
      <div className="flex animate-fade-up flex-col gap-4 motion-reduce:animate-none">
        <MobilePageHeader icon="Truck" title={t('Towing Services')} subtitle={t('Recovery jobs and partner tow operators')} />
        {gap}
      </div>
    )
  }
  ...
  return (
    <div className="flex animate-fade-up flex-col gap-6 motion-reduce:animate-none">
      <PageHeader icon="Truck" title={t('Towing Services')} subtitle={t('Recovery jobs and partner tow operators')} />
      {gap}
    </div>
  )
}
