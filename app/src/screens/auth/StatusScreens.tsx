...
function StatusFrame({
  ...
  return (
    <div className="relative flex min-h-screen items-center justify-center overflow-hidden bg-page font-ui">
      ...
      <div className="relative z-[1] flex max-w-[440px] animate-fade-up flex-col items-center gap-6 p-4 text-center">
        {children}
      </div>
    </div>
  )
...
export function Unauthorized() {
  ...
  return (
    ...
    >
      ...
      <div className="relative h-[120px] w-[120px]">
        ...
        <div className="absolute inset-0 flex items-center justify-center rounded-[20px] border border-border bg-card shadow-lg">
          <div className="flex flex-col items-center gap-1">
            <Icon name="ShieldOff" size={32} className="text-salis-blue" />
            <span className="bg-salis-gradient-r bg-clip-text font-display text-[28px] font-black text-transparent">
              403
            </span>
          </div>
        </div>
      ...
      <div>
        <h1 className="font-display text-2xl font-black text-heading">{t('Unauthorized')}</h1>
        <p className="mt-2 font-action text-sm leading-[1.5] text-muted">
          {t("You don't have permission to access this page.")}
        </p>
      </div>

      <div className="box-border flex w-full items-center gap-3 rounded-lg border border-border bg-card p-4">
  ...
  return (
    ...
    >
      <span className="relative inline-flex h-[76px] w-[76px] items-center justify-center">
        <svg width="76" height="76" className="absolute inset-0 -rotate-90" aria-hidden>
          <circle cx="38" cy="38" r="32" fill="none" stroke="rgba(11,179,255,.15)" strokeWidth="4" />
          <circle
            cx="38"
            cy="38"
            r="32"
        ...
        <Icon name="TimerOff" size={28} className="text-salis-bright" />
      </span>
      ...
      <Link
        to="/login"
        className="mt-2 inline-flex h-11 items-center justify-center whitespace-nowrap rounded bg-salis-gradient px-6 font-action text-sm font-semibold text-white no-underline shadow-[0_4px_12px_rgba(10,94,215,.25)] hover:text-white hover:no-underline"
      >
        {t('Sign In')}
      </Link>
    </StatusFrame>
...
export function AccountLocked() {
  ...
  return (
    ...
    >
      <div className="relative flex h-24 w-24 items-center justify-center rounded-[24px] border-2 border-[rgba(249,115,22,.15)] bg-[rgba(249,115,22,.08)]">
        <Icon name="Lock" size={36} className="text-salis-orange" />
      ...
      <div>
        <h1 className="font-display text-2xl font-black text-heading">{t('Account Locked')}</h1>
        <p className="mt-2 font-action text-sm leading-[1.5] text-muted">
          {t('Your account has been locked for security reasons.')}
        </p>
      </div>

      <div className="box-border flex w-full flex-col gap-2.5 rounded-lg border border-border bg-card p-4">
...
export function LogoutConfirmation() {
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-[rgba(11,31,59,.5)] font-ui">
      <div className="w-full max-w-[360px] animate-fade-up p-4">
        <div className="rounded-lg border border-border bg-card p-6 text-center shadow-lg">
          ...
          <h2 className="font-display text-lg font-bold text-heading">{t('Confirm Logout')}</h2>
