...
/** Language picker + notification opt-in — step 2 of the first-run chain
 *  (Welcome → LanguageSelection → RegionSelection → Login).
 *
...
export function LanguageSelection() {
  ...
  return (
    <AuthLayout step={{ index: 2, of: 4, back: '/welcome' }} className="mx-auto max-w-[460px]">
      <div className={`flex flex-col items-center ${isMobile ? 'gap-4' : 'gap-[22px]'}`}>
        ...
        <div className={`flex w-full flex-col gap-3.5 rounded-lg border border-border bg-card shadow-sm ${isMobile ? 'p-3' : 'p-4'}`}>
          ...
          <button
            ...
            className={cn(
              'flex min-h-[44px] w-full cursor-pointer items-center justify-between gap-3 rounded-[10px] border px-3.5 py-3',
            ...
            <span className="flex items-center gap-2.5">
              <span
                className={cn(
                  ...
                  notifications ? 'bg-salis-blue' : 'bg-border-strong'
                )}
              />
  ...
  return (
    <button
      ...
      className={cn(
        ...
        'font-action transition-all duration-150 hover:-translate-y-0.5 hover:border-salis-blue/40 hover:shadow-lg',
        selected
          ? 'border-salis-blue bg-salis-blue/5 text-salis-blue dark:bg-salis-navy'
