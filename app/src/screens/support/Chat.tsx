 ...
 *  `chat` once the API serves it. */
export function Chat() {
  const { t } = usePreferences()
  const isMobile = useIsMobile()
  const gap = <GapCard icon="MessageSquare" collection="chat" />
  ...
  return (
    <div className="flex animate-fade-up flex-col gap-6 motion-reduce:animate-none">
      <PageHeader icon="MessageSquare" title={t('Live Chat')} subtitle={t('Customer support')} />
      {gap}
    </div>
  )
}
