...
export function Timesheets() {
  ...
  const timesheetColumns: Column<TimesheetRow>[] = [
    { header: 'Date', cell: (r) => r.workDate, code: true },
    { header: 'Clock in', cell: (r) => r.clockIn ?? '—', code: true },
    { header: 'Clock out', cell: (r) => r.clockOut ?? '—', code: true },
    { header: 'Hours', cell: (r) => r.hours.toFixed(2), code: true },
            ...
            <div className="flex flex-col gap-3">
              {grouped.map(([employee, records]) => (
                <div key={employee}>
                  ...
                  <DataTable
                    caption={`Timesheet records for ${employee}`}
                    columns={timesheetColumns}
                    rows={records}
                    rowKey={(r, i) => r._id ?? `${r.workDate}-${i}`}
                    mobileCard={(r) => (
                      <>
                        <MobileCardHeader
                          leading={
