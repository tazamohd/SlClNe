 ...
 *  `loyaltyProgram` once the API serves it. */
export function LoyaltyProgram() {
  const { t } = usePreferences()
  const isMobile = useIsMobile()
  const gap = <GapCard icon="Award" collection="loyaltyProgram" />
  ...
  return (
    <div className="flex animate-fade-up flex-col gap-6 motion-reduce:animate-none">
      <PageHeader icon="Award" title={t('Loyalty Program')} subtitle={t('Member rewards')} />
      {gap}
    </div>
  )
}
