 ...
 *  `dynamicPricing` once the API serves it. */
export function DynamicPricing() {
  const { t } = usePreferences()
  const isMobile = useIsMobile()
  const gap = <GapCard icon="TrendingUp" collection="dynamicPricing" />
  ...
  return (
    <div className="flex animate-fade-up flex-col gap-6 motion-reduce:animate-none">
      <PageHeader icon="TrendingUp" title={t('Dynamic Pricing')} subtitle={t('Pricing rules')} />
      {gap}
    </div>
  )
}
