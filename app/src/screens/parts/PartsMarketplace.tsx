 ...
 *  `partsMarketplace` once the API serves it. */
export function PartsMarketplace() {
  const { t } = usePreferences()
  const isMobile = useIsMobile()
  const gap = <GapCard icon="Store" collection="partsMarketplace" />
  ...
  return (
    <div className="flex animate-fade-up flex-col gap-6 motion-reduce:animate-none">
      <PageHeader icon="Store" title={t('Marketplace')} subtitle={t('Parts marketplace')} />
      {gap}
    </div>
  )
}
