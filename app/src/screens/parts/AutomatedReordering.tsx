 ...
 *  `automatedReordering` once the API serves it. */
export function AutomatedReordering() {
  const { t } = usePreferences()
  const isMobile = useIsMobile()
  const gap = <GapCard icon="RotateCcw" collection="automatedReordering" />
  ...
  return (
    <div className="flex animate-fade-up flex-col gap-6 motion-reduce:animate-none">
      <PageHeader icon="RotateCcw" title={t('Automated Reordering')} subtitle={t('Auto-replenishment rules')} />
      {gap}
    </div>
  )
}
