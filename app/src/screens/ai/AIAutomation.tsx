 ...
 *  `aIAutomation` once the API serves it. */
export function AIAutomation() {
  const { t } = usePreferences()
  const isMobile = useIsMobile()
  const gap = <GapCard icon="Workflow" collection="aIAutomation" />

  if (isMobile) {
    return (
      <div className="flex animate-fade-up flex-col gap-4 motion-reduce:animate-none">
        <MobilePageHeader icon="Workflow" title={t('AI Automation')} subtitle={t('Automation rules and triggers')} />
        {gap}
      </div>
    )
  }
