 ...
 *  `smartPartsRecommendations` once the API serves it. */
export function SmartPartsRecommendations() {
  const { t } = usePreferences()
  const isMobile = useIsMobile()
  const gap = <GapCard icon="Sparkles" collection="smartPartsRecommendations" />

  if (isMobile) {
    return (
      <div className="flex animate-fade-up flex-col gap-4 motion-reduce:animate-none">
        <MobilePageHeader icon="Sparkles" title={t('Smart Parts Recommendations')} subtitle={t('AI-powered parts recommendations for vehicles')} />
        {gap}
      </div>
    )
  }
