 ...
 *  `intelligentPriceOptimizer` once the API serves it. */
export function IntelligentPriceOptimizer() {
  const { t } = usePreferences()
  const isMobile = useIsMobile()
  const gap = <GapCard icon="DollarSign" collection="intelligentPriceOptimizer" />

  if (isMobile) {
    return (
      <div className="flex animate-fade-up flex-col gap-4 motion-reduce:animate-none">
        <MobilePageHeader icon="DollarSign" title={t('Intelligent Price Optimizer')} subtitle={t('AI-powered pricing optimization for services')} />
        {gap}
      </div>
    )
  }
