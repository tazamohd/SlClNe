 ...
 *  `inventoryManagement` once the API serves it. */
export function InventoryManagement() {
  const { t } = usePreferences()
  const isMobile = useIsMobile()
  const gap = <GapCard icon="Package" collection="inventoryManagement" />

  if (isMobile) {
    return (
      <div className="flex animate-fade-up flex-col gap-4 motion-reduce:animate-none">
        <MobilePageHeader icon="Package" title={t('Inventory Management')} subtitle={t('Master inventory dashboard')} />
        {gap}
      </div>
    )
  }
