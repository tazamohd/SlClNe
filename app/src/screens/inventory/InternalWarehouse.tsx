 ...
 *  `internalWarehouse` once the API serves it. */
export function InternalWarehouse() {
  const { t } = usePreferences()
  const isMobile = useIsMobile()
  const gap = <GapCard icon="Warehouse" collection="internalWarehouse" />

  if (isMobile) {
    return (
      <div className="flex animate-fade-up flex-col gap-4 motion-reduce:animate-none">
        <MobilePageHeader icon="Warehouse" title={t('Internal Warehouse')} subtitle={t('Warehouse zones and locations')} />
        {gap}
      </div>
    )
  }
