 ...
 *  UI.LoadingStates.dc.html and UI.FormValidation.dc.html) — reference
 *  galleries for /ui/empty-states, /ui/loading-states and /ui/form-validation.
 *
 *  Render inside the standard `AppShell` like any other screen — page
 *  content only, no chrome of their own. Built from the real pieces
 *  (`EmptyState`, `DataTable`'s own loading skeleton, `Input`) rather than a
 *  bespoke re-implementation, since the whole point of a pattern page is
 *  showing the actual component in its actual states. None of the three
 *  designs shipped a `.Mobile.dc.html` variant, so these just stay legible
 *  down to 390px instead of swapping layouts. Icons come from the shared
 *  registry, with lucide-react imported directly for the handful of names
 *  (AlertCircle, AlertTriangle, CheckCircle, X) the registry doesn't ship. */

// ── shared tone palette — blue = success/valid, orange = warning/error,
//    slate = neutral. Never green/red/yellow/purple/pink/teal (README §7). */
const TONE = {
...
const EMPTY_CASES: readonly EmptyCase[] = [
  {
    ...
    tone: 'blue',
    tag: 'First run',
    when: 'Workshop has no job cards yet',
    icon: 'ClipboardList',
    title: 'No job cards yet',
    body: 'Job cards appear here once a vehicle is checked in. Start with a walk-in or convert an approved estimate.',
    primary: 'New Job Card',
    primaryIcon: 'Plus',
  ...
  },
  {
    ...
    tone: 'slate',
    tag: 'No results',
    when: 'Search or filter returned nothing',
    icon: 'SearchX',
    title: 'No matches found',
    body: 'No records match your search and filters. Try a shorter term, or clear the filters to see everything.',
    primary: 'Clear Filters',
    primaryIcon: 'FilterX',
  },
  {
    key: 'first-run-customers',
    tone: 'blue',
    tag: 'First run',
    when: 'No customers registered',
    icon: 'Users',
    title: 'No customers yet',
    body: 'Add your first customer to start booking appointments and issuing invoices. You can also import an existing list.',
  ...
  },
  {
    ...
    icon: 'CheckCheck',
    title: 'All caught up',
    body: 'No pending approvals or overdue items. New requests will show up here as they arrive.',
  ...
  },
  {
    ...
    tone: 'orange',
    tag: 'Blocked',
    when: 'Integration not connected',
    icon: 'PlugZap',
    title: 'Connect ZATCA to continue',
    body: 'E-invoices cannot be issued until the ZATCA integration is connected and your VAT number is verified.',
    primary: 'Connect',
    primaryIcon: 'Plug',
  ...
  },
  {
    ...
    tone: 'orange',
    tag: 'Error',
    when: 'Data could not be loaded',
    icon: 'CloudOff',
    title: "Couldn't load reports",
    body: 'The request timed out. Your data is safe — retry, or check the service status if this keeps happening.',
    primary: 'Retry',
  ...
  return (
    <>
      ...
      <p className="max-w-2xl text-sm text-muted">
        {t(
          'Six states every list and detail screen needs: first run, no search results, all-clear, blocked by setup, and load failure. Each pairs one clear sentence with the action that resolves it.'
        )}
      </p>
                ...
                action={
                  c.primary || c.secondary ? (
                    <div className="mt-1 flex flex-wrap justify-center gap-2">
                      ...
                      {c.primary ? (
                        <Button size="sm">
                          {c.primaryIcon ? <Icon name={c.primaryIcon} size={14} /> : null}
                          {t(c.primary)}
                        </Button>
                      ) : null}
                    </div>
...
/** Skeletons mirror the shape of the content they replace; progress is
 *  linear and determinate wherever the percentage is known. The CSS
 *  animations here loop harmlessly forever — nothing blocks the headings
 *  above from rendering immediately. */
export function UILoadingStates() {
  ...
  return (
    <>
      ...
      <p className="max-w-2xl text-sm text-muted">
        {t(
          'Skeletons mirror the shape of the content they replace, so layout never jumps when data lands. Progress is linear and determinate wherever the percentage is known.'
        )}
      </p>
      <div className="flex items-center gap-2.5 rounded-lg border border-[rgba(10,94,215,.2)] bg-[rgba(10,94,215,.05)] px-4 py-3">
        ...
        <span className="text-[13px] text-body">
          {t('Brand rule: loading is always a linear bar or a pulse skeleton — never a spinner.')}
        </span>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <Card className="overflow-hidden">
          ...
          <div className="h-[3px] w-full overflow-hidden bg-[rgba(10,94,215,.1)]">
            <div
              className="h-full w-1/3 bg-salis-gradient"
              style={{ animation: 'uiIndeterminate 1.5s ease-in-out infinite' }}
            />
          </div>
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
        ...
        <Card className="flex flex-col gap-3.5 p-5">
          ...
          <div className="h-[3px] overflow-hidden rounded-full bg-[rgba(10,94,215,.1)]">
            <div
              className="h-full w-[30%] rounded-full bg-salis-gradient"
              style={{ animation: 'uiIndeterminate 1.5s ease-in-out infinite' }}
            />
          </div>
        </Card>
...
/** Below-field or below-swatch feedback line. `tone` drives colour — blue for
 *  success/focus, orange for error, muted for a neutral hint. Never green or
 *  red (README §7). */
function Hint({ tone, icon, children }: { tone: 'blue' | 'orange' | 'muted'; icon: ReactNode; children: ReactNode }) {
  ...
  return (
    <span className={cn('flex items-start gap-1.5 text-[11px] leading-[1.5]', cls)}>
      {icon}
      {children}
    </span>
  )
...
function FieldFeedback({ ok, okText, errText }: { ok: boolean; okText: string; errText: string }) {
  return ok ? (
    <Hint tone="blue" icon={<Icon name="Check" size={12} className="mt-px flex-shrink-0" />}>
      {okText}
    </Hint>
  ) : (
    <Hint tone="orange" icon={<AlertCircle size={12} className="mt-px flex-shrink-0" aria-hidden />}>
      {errText}
    </Hint>
  )
}
 ...
 *  Customer" example and the password checklist are wired to real `useState`,
 *  so typing actually flips a field between its error and valid look. */
export function UIFormValidation() {
  ...
  return (
    <>
      ...
      <p className="max-w-2xl text-sm text-muted">
        {t(
          'Field states, inline rules, a summary banner and result toasts. Errors use Action Orange, success and focus use blue — never green or red.'
        )}
      </p>
        ...
        <Card className="flex flex-col gap-4 p-5">
          ...
          <div className="flex flex-col gap-1.5">
            <div className="flex items-center gap-1.5">
              <span className="font-action text-xs font-medium text-heading">{t('Default')}</span>
              <span className="flex-1" />
              <Tag tone="slate">{t('rest')}</Tag>
            </div>
            <Input placeholder={t('Enter a value')} readOnly />
          </div>

          <div className="flex flex-col gap-1.5">
            <div className="flex items-center gap-1.5">
              <span className="font-action text-xs font-medium text-heading">{t('Focused')}</span>
              <span className="flex-1" />
              <Tag tone="blue">{t('focus')}</Tag>
            </div>
            <Input
              defaultValue="Toyota Camry 2022"
              readOnly
              className="border-salis-blue shadow-[0_0_0_3px_rgba(10,94,215,.12)]"
            />
            <Hint tone="muted" icon={<Icon name="Info" size={12} className="mt-px flex-shrink-0" />}>
              {t('Helper text sits below the field and explains the expected format.')}
            </Hint>
          </div>

          <div className="flex flex-col gap-1.5">
            <div className="flex items-center gap-1.5">
              <span className="font-action text-xs font-medium text-heading">{t('Valid')}</span>
              <span className="flex-1" />
              <Tag tone="blue">{t('valid')}</Tag>
            </div>
            <Input
              defaultValue="+966 55 210 4471"
              readOnly
              dir="ltr"
              className="font-mono"
              trailing={<Icon name="Check" size={15} className="text-salis-blue" />}
            />
            <Hint tone="blue" icon={<Icon name="Check" size={12} className="mt-px flex-shrink-0" />}>
              {t('Verified with the customer record.')}
            </Hint>
          </div>

          <div className="flex flex-col gap-1.5">
            <div className="flex items-center gap-1.5">
              <span className="font-action text-xs font-medium text-heading">{t('Error')}</span>
              <span className="flex-1" />
              <Tag tone="orange">{t('invalid')}</Tag>
            </div>
            <Input
              defaultValue="3104"
              readOnly
              ...
              className="font-mono"
              invalid
              trailing={<AlertCircle size={15} className="text-salis-orange" aria-hidden />}
            />
            <Hint tone="orange" icon={<AlertCircle size={12} className="mt-px flex-shrink-0" aria-hidden />}>
              {t('VAT number must be 15 digits and start with 3.')}
            </Hint>
          </div>

          <div className="flex flex-col gap-1.5">
            <div className="flex items-center gap-1.5">
              <span className="font-action text-xs font-medium text-heading">{t('Read-only')}</span>
              <span className="flex-1" />
              <Tag tone="slate">{t('locked')}</Tag>
            </div>
            <Input
              defaultValue="JC-A3F8B2C1"
              readOnly
              disabled
              dir="ltr"
              className="font-mono"
              trailing={<Icon name="Lock" size={15} className="text-muted" />}
            />
            <Hint tone="muted" icon={<Icon name="Lock" size={12} className="mt-px flex-shrink-0" />}>
              {t('Generated automatically and cannot be edited.')}
            </Hint>
          </div>
        </Card>
        ...
        <div className="flex flex-col gap-6">
          ...
          <Card className="overflow-hidden">
            <div className="flex items-center gap-2.5 border-b border-border px-[22px] py-4">
              ...
              <span className="flex-1" />
              <Tag tone={canSave ? 'blue' : 'orange'}>
                {canSave ? t('Ready to save') : `${errorCount} ${t(errorCount === 1 ? 'error' : 'errors')}`}
              </Tag>
            </div>
              ...
              <div className="px-[22px] pt-4">
                <div className="flex gap-2.5 rounded-[10px] border border-[rgba(249,115,22,.22)] bg-[rgba(249,115,22,.06)] px-3.5 py-3">
                  ...
                  <div>
                    ...
                    <p className="mt-0.5 text-xs leading-[1.55] text-body">
                      {t('Some fields are not in a valid format. Fix them to continue.')}
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 gap-4 p-[22px] sm:grid-cols-2">
              <div className="flex flex-col gap-1.5">
                <label className="font-action text-xs font-medium text-heading">
                  {t('Full Name')} <span className="text-salis-orange">*</span>
                </label>
              ...
              <div className="flex flex-col gap-1.5">
                <label className="font-action text-xs font-medium text-heading">
                  {t('Phone')} <span className="text-salis-orange">*</span>
                </label>
                <Input
                  value={phone}
                ...
                <FieldFeedback
                  ok={phoneOk}
                  okText={t('Verified with the customer record.')}
                  errText={t('Enter a Saudi mobile number, e.g. +966 5X XXX XXXX.')}
                />
              </div>
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <Hint tone="muted" icon={<Icon name="Info" size={12} className="mt-px flex-shrink-0" />}>
                  {t('Optional. Required only for VAT-registered business customers.')}
                </Hint>
              </div>
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
            <Card className="flex flex-col gap-3.5 p-5">
              ...
              <Input
                ...
                placeholder={t('Type a password to check the rules')}
                dir="ltr"
              />
                ...
                {PASSWORD_RULES.map((rule) => {
                  ...
                  return (
                    <div key={rule.key} className="flex items-center gap-2">
                      <span
                        className={cn(
                          'flex h-[17px] w-[17px] flex-shrink-0 items-center justify-center rounded-[5px]',
                          ok ? 'bg-[rgba(10,94,215,.12)] text-salis-blue' : 'bg-[rgba(249,115,22,.12)] text-salis-orange'
                        )}
                      >
            ...
            <Card className="flex flex-col gap-2.5 p-5">
              ...
              <div className="flex gap-2.5 rounded-[10px] border border-[rgba(10,94,215,.2)] bg-[rgba(10,94,215,.05)] px-3.5 py-3">
                <CheckCircle size={15} className="mt-px flex-shrink-0 text-salis-blue" aria-hidden />
                <div className="min-w-0 flex-1">
                  <p className="text-[13px] font-semibold text-heading">{t('Customer saved')}</p>
                  <p className="mt-0.5 text-xs leading-[1.5] text-muted">
                    {t('Layla Al-Sulaiman was added and can now be booked.')}
                  </p>
                </div>
                <X size={12} className="mt-0.5 flex-shrink-0 text-muted" aria-hidden />
              </div>

              <div className="flex gap-2.5 rounded-[10px] border border-[rgba(249,115,22,.2)] bg-[rgba(249,115,22,.05)] px-3.5 py-3">
                <AlertTriangle size={15} className="mt-px flex-shrink-0 text-salis-orange" aria-hidden />
                <div className="min-w-0 flex-1">
                  <p className="text-[13px] font-semibold text-heading">{t('Check the highlighted fields')}</p>
                  <p className="mt-0.5 text-xs leading-[1.5] text-muted">
                    {t('Two fields need attention before this form can be saved.')}
                  </p>
                </div>
                <X size={12} className="mt-0.5 flex-shrink-0 text-muted" aria-hidden />
              </div>

              <div className="flex gap-2.5 rounded-[10px] border border-border bg-inset px-3.5 py-3">
                <Icon name="Save" size={15} className="mt-px flex-shrink-0 text-muted" />
                <div className="min-w-0 flex-1">
                  <p className="text-[13px] font-semibold text-heading">{t('Draft saved automatically')}</p>
                  <p className="mt-0.5 text-xs leading-[1.5] text-muted">
                    {t('Your changes are kept for 7 days if you leave this page.')}
                  </p>
                </div>
                <X size={12} className="mt-0.5 flex-shrink-0 text-muted" aria-hidden />
              </div>
