 ...
 *
 *  These are reference galleries, not live dialogs: every variant renders as
 *  an inline "specimen" card, exactly as the design's static HTML does, so a
 *  reviewer sees the whole set on load without opening anything. None of this
 *  is a real overlay — no portal, no backdrop, no focus trap, no click needed.
 *  Companion to ModalsCore.tsx (CRUD / Actions / Status specimens). */
...
function Grid({ children }: { children: ReactNode }) {
  return (
    <div className="grid grid-cols-1 items-start gap-5 md:grid-cols-2 xl:grid-cols-3">{children}</div>
  )
}
  ...
  return (
    <div className="flex flex-shrink-0 justify-end gap-2.5 border-t border-border px-5 py-3.5">
      {actions.map((a) => (
        <Button
          key={a.label}
          variant={a.primary || a.orange ? 'primary' : 'outline'}
          size="sm"
...
/** A dialog's markup shown in place, as a card — icon chip, title, decorative
 *  close glyph, body, optional footer actions. */
function Specimen({
  icon,
  tone = 'blue',
  title,
  ...
  return (
    <Card className="flex flex-col overflow-hidden rounded-2xl">
      <div className="flex flex-shrink-0 items-center gap-2.5 border-b border-border px-5 py-3.5">
        <span className="flex rounded-lg p-1.5" style={{ background: bg, color: fg }}>
...
/** Centered confirm specimen — icon medallion, heading, copy, no header row
 *  (used by the lifecycle activate/deactivate variants). */
function CenteredSpecimen({
  icon,
  tone,
  heading,
  copy,
  ...
  return (
    <Card className="flex flex-col overflow-hidden rounded-2xl">
      <div className="flex flex-col items-center gap-3 px-6 py-7 text-center">
        <span className="flex rounded-full p-3.5" style={{ background: bg, color: fg }}>
...
function SwitchDot({ on }: { on: boolean }) {
  return (
    <span
      aria-hidden
      className={cn(
        'relative inline-block h-[21px] w-[38px] flex-shrink-0 rounded-full transition-colors',
        on ? 'bg-salis-blue' : 'bg-border-strong'
      )}
    >
      <span
        className={cn(
          'absolute top-0.5 h-4 w-4 rounded-full bg-white shadow',
          on ? 'start-[19px]' : 'start-0.5'
        )}
      />
    </span>
  )
...
function Pill({ label, on }: { label: string; on?: boolean }) {
  ...
  return (
    <span
      className={cn(
        'inline-flex h-7 items-center whitespace-nowrap rounded-md px-2.5 font-action text-[11px] font-medium',
        on
          ? 'border border-[rgba(10,94,215,.35)] bg-[rgba(10,94,215,.1)] text-salis-blue'
...
function InsetNote({ icon, tone = 'blue', children }: { icon: string; tone?: Tone; children: ReactNode }) {
  ...
  return (
    <div
      className={cn(
        'flex items-center gap-2 rounded-lg border px-3 py-2.5 text-[11px]',
    ...
    >
      ...
      <span style={tone === 'orange' ? { color: fg } : undefined} className={tone === 'orange' ? '' : 'text-muted'}>
        {children}
      </span>
    </div>
  )
...
const DATA_COLUMNS: readonly [string, boolean][] = [
  ['Job Card', true],
  ['Customer', true],
  ['Vehicle', true],
  ['Technician', true],
...
const DATA_SORTS = [
  { order: '1st', field: 'Created Date', dir: 'DESC', icon: 'ArrowDown' },
  { order: '2nd', field: 'Total', dir: 'ASC', icon: 'ArrowUp' },
] as const

const DATA_SCOPES: readonly [string, boolean][] = [
  ['Job Cards', true],
  ['Customers', true],
  ['Invoices', false],
  ['Vehicles', true],
...
const DATA_FILTERS = [
  ...
  { field: 'Branch', op: 'is', value: 'Riyadh Main' },
  { field: 'Total', op: '>=', value: '1000' },
] as const

const DATA_MAPPINGS = [
  { source: 'full_name', target: 'Customer Name' },
  { source: 'mobile', target: 'Phone' },
  { source: 'email_addr', target: 'Email' },
  { source: 'plate_no', target: 'Vehicle Plate' },
] as const

/** Route /ui/modals/data — column, sort, search, filter, export and import
 *  dialog variants. */
  ...
  return (
    <>
      <FeatureHeader
        ...
        title={t('Data Modals')}
        subtitle={t('Column, sort, search, filter, export and import surfaces')}
      />
      <Grid>
        ...
        <Specimen icon="ArrowUpDown" tone="cyan" title="Sort Options" actions={[{ label: 'Clear' }, { label: 'Apply Sort', primary: true }]}>
          ...
          <button
            type="button"
            className="flex h-[34px] cursor-pointer items-center justify-center gap-1.5 rounded-lg border border-dashed border-border bg-transparent font-action text-xs text-salis-blue"
          >
        ...
        <Specimen icon="SearchCode" tone="navy" title="Advanced Search" actions={[{ label: 'Reset' }, { label: 'Search', primary: true }]}>
          <Field label="Keywords">
            <Input placeholder={t('Search across all records...')} readOnly inputSize="md" />
          </Field>
          <div className="grid grid-cols-2 gap-2.5">
            <Field label="From">
              <Input value="2026-07-01" readOnly dir="ltr" inputSize="md" className="font-mono text-xs" />
            </Field>
            <Field label="To">
              <Input value="2026-07-24" readOnly dir="ltr" inputSize="md" className="font-mono text-xs" />
            </Field>
          </div>
          <Field label="Search in">
            <div className="flex flex-wrap gap-1.5">
              {DATA_SCOPES.map(([label, on]) => (
                <Pill key={label} label={label} on={on} />
              ))}
            </div>
          </Field>
        </Specimen>
        ...
        <Specimen icon="SlidersHorizontal" tone="orange" title="Filter Builder" actions={[{ label: 'Reset' }, { label: 'Apply Filters', primary: true }]}>
          ...
          <button
            type="button"
            className="flex h-8 cursor-pointer items-center justify-center gap-1.5 rounded-lg border border-dashed border-border bg-transparent font-action text-xs text-salis-blue"
          >
            <Icon name="Plus" size={13} />
            {t('Add condition')}
          </button>
        ...
        <Specimen icon="Download" tone="blue" title="Export Records" actions={[{ label: 'Cancel' }, { label: 'Export', primary: true }]}>
          <Field label="Format">
            <div className="flex gap-2">
              {[
                { label: 'CSV', icon: 'FileText', on: true },
                { label: 'Excel', icon: 'FileSpreadsheet', on: false },
                { label: 'PDF', icon: 'FileType', on: false },
              ].map((f) => (
                <span
                  key={f.label}
                  className={cn(
                    'flex flex-1 items-center justify-center gap-1.5 rounded-lg border-[1.5px] py-2 font-action text-xs font-medium',
          ...
          <Field label="Scope">
            <div className="flex flex-col gap-1.5">
              {[
                ['All records (42)', true],
                ['Current filter (18)', false],
                ['Selected only (5)', false],
              ].map(([label, checked]) => (
                <span key={label as string} className="flex items-center gap-2 text-[13px] text-body">
                  <span
                    aria-hidden
                    className={cn(
                      'flex h-[15px] w-[15px] flex-shrink-0 items-center justify-center rounded-full border-[1.5px]',
                      checked ? 'border-salis-blue' : 'border-border-strong'
                    )}
                  >
 ...
 *  block: no device media is requested and no remote asset is loaded. */
export function UIModalsCapture() {
  ...
  return (
    <>
      <FeatureHeader
        icon="Camera"
        title={t('Capture & Print Modals')}
        subtitle={t('Photo capture, barcode/QR scan, signature and print surfaces')}
      />
      <Grid>
        <Specimen icon="Camera" tone="blue" title="Capture Photo">
          <div className="relative flex aspect-[4/3] items-center justify-center overflow-hidden rounded-xl border border-border bg-gradient-to-br from-[rgba(11,31,59,.12)] to-[rgba(10,94,215,.06)]">
            ...
            <span className="absolute top-3 start-3 rounded-full bg-[rgba(249,115,22,.9)] px-2 py-0.5 text-[10px] font-semibold text-white">
              {t('LIVE')}
            </span>
          </div>
          <div className="flex items-center justify-center gap-3">
            <span className="flex h-10 w-10 items-center justify-center rounded-[10px] border border-border bg-inset text-body">
              <Icon name="Image" size={17} />
            </span>
            <span className="flex h-14 w-14 items-center justify-center rounded-full border-[3px] border-salis-blue bg-salis-gradient text-white shadow-[0_6px_16px_rgba(10,94,215,.3)]">
              <Icon name="Camera" size={22} />
            </span>
            <span className="flex h-10 w-10 items-center justify-center rounded-[10px] border border-border bg-inset text-body">
              <Icon name="SwitchCamera" size={17} />
            </span>
          </div>
        ...
        <Specimen icon="ScanBarcode" tone="cyan" title="Scan Barcode" actions={[{ label: 'Cancel' }, { label: 'Add to Job', primary: true }]}>
          <div className="relative flex aspect-[4/3] items-center justify-center overflow-hidden rounded-xl border border-border bg-gradient-to-br from-[rgba(11,179,255,.1)] to-[rgba(10,94,215,.05)]">
            <svg viewBox="0 0 160 60" className="w-[62%] opacity-50" aria-hidden>
              <g fill="#0B1F3B">
                <rect x="4" y="6" width="3" height="48" />
                <rect x="11" y="6" width="2" height="48" />
                <rect x="17" y="6" width="5" height="48" />
                <rect x="26" y="6" width="2" height="48" />
                <rect x="32" y="6" width="4" height="48" />
                <rect x="40" y="6" width="2" height="48" />
                <rect x="46" y="6" width="6" height="48" />
                <rect x="56" y="6" width="2" height="48" />
                <rect x="62" y="6" width="3" height="48" />
                <rect x="69" y="6" width="5" height="48" />
                <rect x="78" y="6" width="2" height="48" />
                <rect x="84" y="6" width="4" height="48" />
              </g>
            </svg>
        ...
        <Specimen icon="QrCode" tone="navy" title="Scan QR Code">
          <div className="relative flex aspect-square items-center justify-center rounded-xl border border-border bg-gradient-to-br from-[rgba(11,31,59,.08)] to-[rgba(10,94,215,.05)]">
            ...
            {(['top-4 start-4 border-t-[3px] border-s-[3px] rounded-tl-md', 'top-4 end-4 border-t-[3px] border-e-[3px] rounded-tr-md', 'bottom-4 start-4 border-b-[3px] border-s-[3px] rounded-bl-md', 'bottom-4 end-4 border-b-[3px] border-e-[3px] rounded-br-md'] as const).map((pos) => (
              <span key={pos} className={cn('absolute h-[26px] w-[26px] border-salis-blue', pos)} />
            ))}
          </div>
        ...
        <Specimen icon="PenTool" tone="orange" title="Customer Signature" actions={[{ label: 'Cancel' }, { label: 'Accept Signature', primary: true }]}>
          <div className="relative flex h-[150px] items-center justify-center rounded-xl border-[1.5px] border-dashed border-border bg-inset">
            <svg viewBox="0 0 320 120" className="h-auto w-[88%]" aria-hidden>
              <path
                d="M24,84 C48,44 62,96 84,66 C104,38 118,92 140,70 C158,52 168,88 188,68 C206,50 220,84 244,58 C262,38 280,66 298,52"
                fill="none"
                stroke="#0B1F3B"
                strokeWidth="2.4"
                strokeLinecap="round"
          ...
          <div className="flex items-center gap-2.5">
            <span className="flex h-9 flex-1 items-center justify-center rounded-lg border border-border bg-card font-action text-xs font-medium text-body">
              {t('Clear')}
            </span>
        ...
        <Specimen icon="Printer" tone="blue" title="Print Preview" actions={[{ label: 'Cancel' }, { label: 'Print', primary: true }]}>
          <div className="flex aspect-[1/1.3] flex-col gap-2 overflow-hidden rounded-lg border border-border bg-white p-4 shadow-lg">
            ...
            {[
              ...
              <div key={i} className="rounded-sm bg-[#E2E8F0]" style={{ height: l.h, width: `${l.w}%` }} />
            ))}
          ...
          <div className="flex gap-2">
            <span className="flex h-[34px] flex-1 items-center rounded-lg border border-border bg-inset px-2.5 text-xs text-body">
              {t('A4 · Portrait')}
            </span>
            <span className="flex h-[34px] flex-1 items-center rounded-lg border border-border bg-inset px-2.5 text-xs text-body">
              {t('1 copy')}
...
const LIFECYCLE_BRANCHES = [
  { name: 'Riyadh North', detail: 'Al-Nakheel · 2 bays free', on: true },
  { name: 'Jeddah Branch', detail: 'Al-Safa · 1 bay free', on: false },
  { name: 'Dammam Branch', detail: 'Al-Faisaliah · 3 bays free', on: false },
] as const

/** Route /ui/modals/lifecycle — transfer, merge, restore, activation and lock
 *  workflow dialog variants. */
export function UIModalsLifecycle() {
  ...
  return (
    <>
      <FeatureHeader
        icon="ArrowRightLeft"
        title={t('Lifecycle Modals')}
        subtitle={t('Transfer, merge, restore, activation and lock workflows')}
      />
      <Grid>
            ...
            <div className="flex flex-col gap-2">
              {LIFECYCLE_BRANCHES.map((b) => (
                <div
                  key={b.name}
                  className={cn(
                    'flex items-center gap-2.5 rounded-lg border-[1.5px] px-3 py-2.5',
                    b.on ? 'border-salis-blue bg-[rgba(10,94,215,.05)]' : 'border-border bg-inset'
                  )}
                >
        ...
        <Specimen icon="Merge" tone="cyan" title="Merge Customers" actions={[{ label: 'Cancel' }, { label: 'Merge Records', primary: true }]}>
          <p className="max-w-[330px] text-sm leading-relaxed text-muted">
            {t('Two records look like the same customer. Choose which one to keep as the primary.')}
          </p>
          {[
            { name: 'Ahmed Al-Rashid', initial: 'A', detail: '+966 55 210 4471 · 3 vehicles · 12 invoices', tag: 'Primary', on: true },
            { name: 'Ahmed AlRashid', initial: 'A', detail: '+966 55 210 4471 · 0 vehicles · 1 invoice', tag: 'Duplicate', on: false },
          ].map((m) => (
            <div
              key={m.name}
              className={cn(
                'flex items-center gap-2.5 rounded-lg border-[1.5px] px-3 py-2.5',
                m.on ? 'border-salis-blue bg-[rgba(10,94,215,.05)]' : 'border-border bg-inset'
              )}
            >
              ...
              <span
                className={cn(
                  ...
                  m.on ? 'bg-[rgba(10,94,215,.12)] text-salis-blue' : 'bg-[rgba(100,116,139,.1)] text-muted'
                )}
              >
                {t(m.tag)}
              </span>
        ...
        <Specimen icon="ArchiveRestore" tone="blue" title="Restore from Archive" actions={[{ label: 'Cancel' }, { label: 'Restore', primary: true }]}>
          <p className="max-w-[330px] text-sm leading-relaxed text-muted">
            {t('Restore this archived record and return it to the active list.')}
          </p>
          <div className="flex items-center gap-2.5 rounded-lg border border-border bg-inset px-3 py-2.5">
            <span className="flex flex-shrink-0 rounded-lg bg-[rgba(100,116,139,.1)] p-1.5 text-[#64748B]">
        ...
        <Specimen icon="Lock" tone="navy" title="Lock Record" actions={[{ label: 'Cancel' }, { label: 'Lock Record', primary: true }]}>
          <p className="max-w-[330px] text-sm leading-relaxed text-muted">
            {t('Locking prevents further edits. Only managers and above can unlock it again.')}
          </p>
          <Field label="Reason">
            <textarea
              rows={3}
              readOnly
              placeholder={t('Why is this record being locked?')}
