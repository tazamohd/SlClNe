 ...
 *  `salesGuide` once the API serves it. */
export function SalesGuide() {
  const { t } = usePreferences()
  const isMobile = useIsMobile()
  const gap = <GapCard icon="BookOpen" collection="salesGuide" />
  ...
  return (
    <div className="flex animate-fade-up flex-col gap-6 motion-reduce:animate-none">
      <PageHeader icon="BookOpen" title={t('Sales Guide')} subtitle={t('Sales methodology')} />
      {gap}
    </div>
  )
}
