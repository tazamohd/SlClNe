 ...
 *  `nextGenTechnologies` once the API serves it. */
export function NextGenTechnologies() {
  const { t } = usePreferences()
  const isMobile = useIsMobile()
  const gap = <GapCard icon="Layers" collection="nextGenTechnologies" />

  if (isMobile) {
    return (
      <div className="flex animate-fade-up flex-col gap-4 motion-reduce:animate-none">
        <MobilePageHeader icon="Layers" title={t('Next-Gen Technologies')} subtitle={t('Technology roadmap and innovation pipeline')} />
        {gap}
      </div>
    )
  }
