 ...
 *  `emergingTechnologies` once the API serves it. */
export function EmergingTechnologies() {
  const { t } = usePreferences()
  const isMobile = useIsMobile()
  const gap = <GapCard icon="Cpu" collection="emergingTechnologies" />
  ...
  return (
    <div className="flex animate-fade-up flex-col gap-6 motion-reduce:animate-none">
      <PageHeader icon="Cpu" title={t('Emerging Tech')} subtitle={t('Technology Overview')} />
      {gap}
    </div>
  )
}
