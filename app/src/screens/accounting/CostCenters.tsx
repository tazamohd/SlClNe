 ...
 *  `costCenters` once the API serves it. */
export function CostCenters() {
  const { t } = usePreferences()
  const isMobile = useIsMobile()
  const gap = <GapCard icon="Target" collection="costCenters" />

  if (isMobile) {
    return (
      <div className="flex animate-fade-up flex-col gap-4 motion-reduce:animate-none">
        <MobilePageHeader icon="Target" title={t('Cost Centers')} subtitle={t('Budget tracking by department and cost center')} />
        {gap}
      </div>
    )
  }
  ...
  return (
    <div className="flex animate-fade-up flex-col gap-6 motion-reduce:animate-none">
      <PageHeader icon="Target" title={t('Cost Centers')} subtitle={t('Budget tracking by department and cost center')} />
      {gap}
    </div>
  )
}
