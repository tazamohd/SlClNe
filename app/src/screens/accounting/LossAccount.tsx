 ...
 *  `lossAccount` once the API serves it. */
export function LossAccount() {
  const { t } = usePreferences()
  const isMobile = useIsMobile()
  const gap = <GapCard icon="BarChart3" collection="lossAccount" />

  if (isMobile) {
    return (
      <div className="flex animate-fade-up flex-col gap-4 motion-reduce:animate-none">
        <MobilePageHeader icon="BarChart3" title={t('Profit & Loss')} subtitle={t('Revenue and expense breakdown with net profit')} />
        {gap}
      </div>
    )
  }
  ...
  return (
    <div className="flex animate-fade-up flex-col gap-6 motion-reduce:animate-none">
      <PageHeader icon="BarChart3" title={t('Profit & Loss')} subtitle={t('Revenue and expense breakdown with net profit')} />
      {gap}
    </div>
  )
}
