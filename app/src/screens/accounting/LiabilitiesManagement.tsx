 ...
 *  `liabilitiesManagement` once the API serves it. */
export function LiabilitiesManagement() {
  const { t } = usePreferences()
  const isMobile = useIsMobile()
  const gap = <GapCard icon="Scale" collection="liabilitiesManagement" />

  if (isMobile) {
    return (
      <div className="flex animate-fade-up flex-col gap-4 motion-reduce:animate-none">
        <MobilePageHeader icon="Scale" title={t('Liabilities Management')} subtitle={t('Current and long-term obligations')} />
        {gap}
      </div>
    )
  }
  ...
  return (
    <div className="flex animate-fade-up flex-col gap-6 motion-reduce:animate-none">
      <PageHeader icon="Scale" title={t('Liabilities Management')} subtitle={t('Current and long-term obligations')} />
      {gap}
    </div>
  )
}
