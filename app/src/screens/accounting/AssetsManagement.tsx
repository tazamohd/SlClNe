 ...
 *  `assetsManagement` once the API serves it. */
export function AssetsManagement() {
  const { t } = usePreferences()
  const isMobile = useIsMobile()
  const gap = <GapCard icon="Building" collection="assetsManagement" />

  if (isMobile) {
    return (
      <div className="flex animate-fade-up flex-col gap-4 motion-reduce:animate-none">
        <MobilePageHeader icon="Building" title={t('Assets Management')} subtitle={t('Fixed asset register with depreciation tracking')} />
        {gap}
      </div>
    )
  }
  ...
  return (
    <div className="flex animate-fade-up flex-col gap-6 motion-reduce:animate-none">
      <PageHeader icon="Building" title={t('Assets Management')} subtitle={t('Fixed asset register with depreciation tracking')} />
      {gap}
    </div>
  )
}
