...
**Status:** GENERATED · **Generated:** 2026-09-13 · 150 requirements
...
| --- | --- | --- | --- | --- | --- |
| FR-WORKSHOP-001 | The system provides workshop operations through 16 screens and 61 API endpoints, gated by the `jobcards`, `appointments`, `estimates` permission modules. | CAP-WORKSHOP | 61 | 16 | `project-control/CAPABILITY_REGISTRY.json` |
| FR-CUSTOMERS-001 | The system provides customer management through 3 screens and 15 API endpoints, gated by the `customers` permission module. | CAP-CUSTOMERS | 15 | 3 | `project-control/CAPABILITY_REGISTRY.json` |
| FR-VEHICLES-001 | The system provides vehicle management through 4 screens and 7 API endpoints, gated by the `vehicles` permission module. | CAP-VEHICLES | 7 | 4 | `project-control/CAPABILITY_REGISTRY.json` |
| FR-INVENTORY-001 | The system provides parts and inventory through 7 screens and 11 API endpoints, gated by the `inventory` permission module. | CAP-INVENTORY | 11 | 7 | `project-control/CAPABILITY_REGISTRY.json` |
| FR-PROCUREMENT-001 | The system provides procurement through 1 screens and 24 API endpoints, gated by the `procurement` permission module. | CAP-PROCUREMENT | 24 | 1 | `project-control/CAPABILITY_REGISTRY.json` |
| FR-BILLING-001 | The system provides invoicing and payments through 6 screens and 20 API endpoints, gated by the `invoices`, `payments` permission modules. | CAP-BILLING | 20 | 6 | `project-control/CAPABILITY_REGISTRY.json` |
| FR-ACCOUNTING-001 | The system provides accounting and finance through 7 screens and 40 API endpoints, gated by the `accounting` permission module. | CAP-ACCOUNTING | 40 | 7 | `project-control/CAPABILITY_REGISTRY.json` |
| FR-HR-001 | The system provides hr and payroll through 5 screens and 41 API endpoints, gated by the `hr`, `technicians` permission modules. | CAP-HR | 41 | 5 | `project-control/CAPABILITY_REGISTRY.json` |
| FR-CRM-001 | The system provides crm and sales through 12 screens and 35 API endpoints, gated by the `crm`, `callcenter` permission modules. | CAP-CRM | 35 | 12 | `project-control/CAPABILITY_REGISTRY.json` |
...
| FR-GOVERNANCE-001 | The system provides approvals and governance through 2 screens and 4 API endpoints, gated by the `approvals`, `audit` permission modules. | CAP-GOVERNANCE | 4 | 2 | `project-control/CAPABILITY_REGISTRY.json` |
...
| FR-AI-001 | The system provides ai and automation through 10 screens and 6 API endpoints, gated by the `ai` permission module. | CAP-AI | 6 | 10 | `project-control/CAPABILITY_REGISTRY.json` |
| FR-PLATFORM-001 | The system provides administration and platform through 36 screens and 15 API endpoints, gated by the `admin`, `settings`, `dashboard`, `network`, `ungated`, `platform` permission modules. | CAP-PLATFORM | 15 | 36 | `project-control/CAPABILITY_REGISTRY.json` |
...
| FR-WEBSITE-001 | The system provides public website and acquisition through 31 screens and 0 API endpoints, gated by the domain permission modules. | CAP-WEBSITE | — | 31 | `project-control/CAPABILITY_REGISTRY.json` |
...
| NFR-I18N-001 | Arabic and RTL are verified per screen, and RTL hazards are held at zero. | project-control/STATUS.json — arabicVerified 82 of 424, rtlHazards 0 | MEASURED_REGISTRY |
| NFR-UX-001 | Every registered capability renders and has an end-to-end assertion on its content, not merely on its route. | project-control/STATUS.json — contentAsserted 424 of 424 | MEASURED_REGISTRY |
