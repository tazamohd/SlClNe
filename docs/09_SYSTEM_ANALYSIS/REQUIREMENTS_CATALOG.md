...
| FR-INVENTORY-001 | The system provides parts and inventory through 9 screens and 22 API endpoints, gated by the `inventory` permission module. | CAP-INVENTORY | 22 | 9 | `project-control/CAPABILITY_REGISTRY.json` |
...
| FR-ACCOUNTING-001 | The system provides accounting and finance through 12 screens and 59 API endpoints, gated by the `accounting`, `insurance` permission modules. | CAP-ACCOUNTING | 59 | 12 | `project-control/CAPABILITY_REGISTRY.json` |
| FR-HR-001 | The system provides hr and payroll through 13 screens and 71 API endpoints, gated by the `hr`, `technicians` permission modules. | CAP-HR | 71 | 13 | `project-control/CAPABILITY_REGISTRY.json` |
...
| FR-PLATFORM-001 | The system provides administration and platform through 35 screens and 73 API endpoints, gated by the `admin`, `departments`, `settings`, `superadmin`, `dashboard`, `network`, `ungated`, `platform` permission modules. | CAP-PLATFORM | 73 | 35 | `project-control/CAPABILITY_REGISTRY.json` |
| FR-IDENTITY-001 | The system provides identity and access through 19 screens and 28 API endpoints, gated by the `auth` permission module. | CAP-IDENTITY | 28 | 19 | `project-control/CAPABILITY_REGISTRY.json` |
...
| FR-DESIGNSYSTEM-001 | The system provides design system and reference surfaces through 198 screens and 0 API endpoints, gated by the domain permission modules. | CAP-DESIGNSYSTEM | — | 198 | `project-control/CAPABILITY_REGISTRY.json` |
...
| NFR-I18N-001 | Arabic and RTL are verified per screen, and RTL hazards are held at zero. | project-control/STATUS.json — arabicVerified 122 of 436, rtlHazards 0 | MEASURED_REGISTRY |
...
| DR-JOB-CARDS | `job_cards` holds 22 columns, is tenant-scoped on `org_id` and is protected by row-level security. |
| DR-APPOINTMENTS | `appointments` holds 23 columns, is tenant-scoped on `org_id` and is protected by row-level security. |
| DR-ESTIMATES | `estimates` holds 28 columns, is tenant-scoped on `org_id` and is protected by row-level security. |
| DR-ESTIMATE-LINES | `estimate_lines` holds 17 columns, is tenant-scoped on `org_id` and is protected by row-level security. |
| DR-DECLINED-JOBS | `declined_jobs` holds 27 columns, is tenant-scoped on `org_id` and is protected by row-level security. |
...
| DR-PARTS | `parts` holds 18 columns, is tenant-scoped on `org_id` and is protected by row-level security. |
...
| DR-WAREHOUSE-ZONES | `warehouse_zones` holds 17 columns, is tenant-scoped on `org_id` and is protected by row-level security. |
| DR-NOTIFICATIONS | `notifications` holds 15 columns, is tenant-scoped on `org_id` and is protected by row-level security. |
| DR-PARTS-NETWORK-MEMBERS | `parts_network_members` holds 21 columns, is tenant-scoped on `org_id` and is protected by row-level security. |
| DR-PARTS-NETWORK-REQUESTS | `parts_network_requests` holds 27 columns, is tenant-scoped on `org_id` and is protected by row-level security. |
| DR-PARTS-NETWORK-QUOTATIONS | `parts_network_quotations` holds 22 columns, is tenant-scoped on `org_id` and is protected by row-level security. |
| DR-PARTS-NETWORK-ORDERS | `parts_network_orders` holds 26 columns, is tenant-scoped on `org_id` and is protected by row-level security. |
