...
**Endpoint-to-test matching is by path string.** 282 of 372 endpoints have no spec file naming their path. A test reaching an endpoint through a helper or a golden path does not match, so this over-reports — and it is still the right number to drive down.

**2 rule guard(s) have no test naming them**, and 6 capabilities have no linked test suite.
