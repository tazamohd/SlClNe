...
| Endpoint → entity | 249 | 390 | Table name via the collection definition |
| Endpoint → permission | 361 | 390 | `requirePermission` call in the handler |
...
**Endpoint-to-test matching is by path string.** 280 of 390 endpoints have no spec file naming their path. A test reaching an endpoint through a helper or a golden path does not match, so this over-reports — and it is still the right number to drive down.
