...
| Capability → endpoint | 529 | 529 | Permission module |
| Capability → screen | 436 | 436 | Permission module, or screen domain where the screen has none |
| Endpoint → entity | 358 | 529 | Table name via the collection definition |
| Endpoint → permission | 496 | 529 | `requirePermission` call in the handler |
...
**Endpoint-to-test matching is by path string.** 386 of 529 endpoints have no spec file naming their path. A test reaching an endpoint through a helper or a golden path does not match, so this over-reports — and it is still the right number to drive down.
