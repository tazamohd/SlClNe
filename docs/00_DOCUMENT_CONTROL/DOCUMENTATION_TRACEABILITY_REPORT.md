...
| Capability → endpoint | 509 | 509 | Permission module |
| Capability → screen | 436 | 436 | Permission module, or screen domain where the screen has none |
| Endpoint → entity | 342 | 509 | Table name via the collection definition |
| Endpoint → permission | 477 | 509 | `requirePermission` call in the handler |
...
**Endpoint-to-test matching is by path string.** 372 of 509 endpoints have no spec file naming their path. A test reaching an endpoint through a helper or a golden path does not match, so this over-reports — and it is still the right number to drive down.
