...
| Capability → endpoint | 423 | 423 | Permission module |
| Capability → screen | 429 | 429 | Permission module, or screen domain where the screen has none |
| Endpoint → entity | 273 | 423 | Table name via the collection definition |
| Endpoint → permission | 394 | 423 | `requirePermission` call in the handler |
...
**Endpoint-to-test matching is by path string.** 308 of 423 endpoints have no spec file naming their path. A test reaching an endpoint through a helper or a golden path does not match, so this over-reports — and it is still the right number to drive down.
