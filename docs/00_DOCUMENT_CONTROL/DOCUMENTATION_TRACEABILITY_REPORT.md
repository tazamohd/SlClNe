...
| Capability → endpoint | 390 | 390 | Permission module |
| Capability → screen | 433 | 433 | Permission module, or screen domain where the screen has none |
| Endpoint → entity | 254 | 390 | Table name via the collection definition |
| Endpoint → permission | 363 | 390 | `requirePermission` call in the handler |
...
**Endpoint-to-test matching is by path string.** 281 of 390 endpoints have no spec file naming their path. A test reaching an endpoint through a helper or a golden path does not match, so this over-reports — and it is still the right number to drive down.
