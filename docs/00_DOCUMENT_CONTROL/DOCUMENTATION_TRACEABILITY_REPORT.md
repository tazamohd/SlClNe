...
| Capability → endpoint | 449 | 449 | Permission module |
| Capability → screen | 430 | 430 | Permission module, or screen domain where the screen has none |
| Endpoint → entity | 297 | 449 | Table name via the collection definition |
| Endpoint → permission | 422 | 449 | `requirePermission` call in the handler |
...
**Endpoint-to-test matching is by path string.** 329 of 449 endpoints have no spec file naming their path. A test reaching an endpoint through a helper or a golden path does not match, so this over-reports — and it is still the right number to drive down.
