...
| Capability → endpoint | 303 | 303 | Permission module |
| Capability → screen | 424 | 424 | Permission module, or screen domain where the screen has none |
| Endpoint → entity | 172 | 303 | Table name via the collection definition |
| Endpoint → permission | 276 | 303 | `requirePermission` call in the handler |
...
**Endpoint-to-test matching is by path string.** 261 of 303 endpoints have no spec file naming their path. A test reaching an endpoint through a helper or a golden path does not match, so this over-reports — and it is still the right number to drive down.

**2 rule guard(s) have no test naming them**, and 10 capabilities have no linked test suite.
