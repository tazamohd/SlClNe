...
| Endpoints documented | 529 of 529 |
| Endpoints with a linked test | 143 of 529 |
...
| Screens wired to the live API | 177 of 436 |
| Test suites catalogued | 233 containing 2546 cases |
...
### 3. 27 authenticated endpoints state no permission guard in the handler
...
| GET | `/api/v1/security/summary` | `server/src/routes/security.ts` |
...
### 5. 10 screens read design fixtures rather than the API

Measured in `project-control/STATUS.json`, not asserted here. Every screen renders and every screen has a content assertion — and 10 of 436 are not yet connected to live data.
...
| `docs/17_API_INTEGRATION/endpoints/platform.md` | 1138 |
...
| `docs/33_MASTER_DIAGRAM_LIBRARY/ERD/INVENTORY_ERD.md` | 911 |
...
3. **Confirm the 27 endpoints with no stated guard.** Each is either guarded through a helper (fix the documentation) or genuinely open (fix the code).
...
6. **Connect the remaining 10 screens to the API**, which is the bulk of the product work still outstanding.
