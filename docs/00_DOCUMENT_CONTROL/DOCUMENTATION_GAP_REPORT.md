...
| Endpoints documented | 446 of 446 |
| Endpoints with a linked test | 120 of 446 |
...
| Screens wired to the live API | 145 of 436 |
| Test suites catalogued | 206 containing 2326 cases |
...
### 4. 326 endpoints have no test matched to them by path
...
### 5. 237 screens read design fixtures rather than the API

Measured in `project-control/STATUS.json`, not asserted here. Every screen renders and every screen has a content assertion — and 237 of 436 are not yet connected to live data.
...
4. **Drive the 326 path-unmatched endpoints down**, starting with the write endpoints that move money or stock.
...
6. **Connect the remaining 237 screens to the API**, which is the bulk of the product work still outstanding.
