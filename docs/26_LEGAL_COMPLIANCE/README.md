<!-- GENERATED FILE — DO NOT EDIT BY HAND.
     Generator: tools/docs/generators/control.mjs
     Regenerate: node tools/docs/generate.mjs
     Derived from:
       - the docs/ tree
       - tools/docs/lib/structure.mjs
-->

# Legal and compliance

**Status:** GENERATED · **Sources as of:** 2026-09-15

ZATCA, VAT, privacy, retention — system requirements, not legal advice.

## In this section

_Nothing yet. Material for this section is listed below, or has not been written._

## Related material not yet migrated

| Where it is now | What it covers |
| --- | --- |
| `docs/legal/` | Terms, privacy policy, DPA, EULA, acceptable use, cookie policy |
| `docs/knowledge-base/library/zatca-compliance-checklist.md` | ZATCA checklist |
| `docs/management/compliance-management-plan.md` | Compliance management plan |

These are classified in the [migration manifest](../00_DOCUMENT_CONTROL/DOCUMENTATION_MIGRATION_MANIFEST.md) and move one section at a time so each change stays reviewable.

[← Documentation index](../00_DOCUMENT_CONTROL/DOCS_INDEX.md)
