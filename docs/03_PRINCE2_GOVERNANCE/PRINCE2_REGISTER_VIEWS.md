...
| F-039 | LOW | The departments collection has no write routes at all — Add Department 404s for every role | server/src/registry.ts's departments define() block carries no `writable: true` (unlike collections such as branches or suppliers that do), and server/src/routes/collections.ts:182 only registers crea |
