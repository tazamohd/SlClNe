...
| PR-04 | Screens are wired to the live API rather than design fixtures | **no** | mockOnly: 223 of 430 |
| PR-05 | Tenant isolation covers every tenant-scoped table | yes | 73 tables with RLS, all FORCEd; 0 uncovered |
...
| PR-12 | Every endpoint has a test matched to it | **no** | 329 of 449 unmatched by path |
...
The product is much further along than a 8-of-15 score suggests, and the score is still the right one to publish. Every screen exists, renders, and is asserted; isolation, authorization and audit are enforced by the database and the server rather than by convention. What is not done is the part that only shows up in production: **223 of 430 screens still read design fixtures**, so a large share of the product has never exchanged a byte with the API under real conditions.
