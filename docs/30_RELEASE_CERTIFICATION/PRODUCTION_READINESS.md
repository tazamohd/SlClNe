...
| PR-02 | Every screen has an end-to-end assertion on its content | yes | contentAsserted: 424 of 424 |
| PR-03 | Golden paths pass | yes | 23 passing, 0 failing, 0 unwritten |
| PR-04 | Screens are wired to the live API rather than design fixtures | **no** | mockOnly: 285 of 424 |
...
| PR-10 | Arabic is verified on every screen | **no** | arabicVerified: 82 of 424 |
| PR-11 | Tablet layouts are verified | **no** | tabletVerified: 4 of 424 |
| PR-12 | Every endpoint has a test matched to it | **no** | 282 of 372 unmatched by path |
...
The product is much further along than a 8-of-15 score suggests, and the score is still the right one to publish. Every screen exists, renders, and is asserted; isolation, authorization and audit are enforced by the database and the server rather than by convention. What is not done is the part that only shows up in production: **285 of 424 screens still read design fixtures**, so a large share of the product has never exchanged a byte with the API under real conditions.
