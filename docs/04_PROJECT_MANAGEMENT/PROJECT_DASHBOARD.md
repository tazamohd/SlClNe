...
| Reading design fixtures | 274 of 429 | `STATUS.json` |
...
| API endpoints | 407 | the route files |
| Test cases | 2234 | the spec files |
...
**274 of 429 screens still read design fixtures rather than the API.** Everything else on this dashboard looks healthier than the project is, because "renders and is asserted" is a genuine achievement that is not the same as "works against the server". Read every other row against that one.
