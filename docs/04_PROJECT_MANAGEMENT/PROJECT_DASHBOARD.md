...
| Reading design fixtures | 275 of 428 | `STATUS.json` |
...
| Test cases | 2219 | the spec files |
| Open risks | 10 of 11 | `RISK_REGISTER.json` |
| Open blockers | 2 | `BLOCKERS.json` |
| Unresolved findings | 10 of 36 | `FINDINGS.json` |
| Release gates passing | 10 of 14 | `RELEASE_GATES.json` |

## The one number that matters most

**275 of 428 screens still read design fixtures rather than the API.** Everything else on this dashboard looks healthier than the project is, because "renders and is asserted" is a genuine achievement that is not the same as "works against the server". Read every other row against that one.

## Open blockers

| ID | Severity | Title | What to do | Owner |
| --- | --- | --- | --- | --- |
| BLK-004 | CRITICAL | 275 rendered capabilities are mock-only | They render, but read fixtures rather than an API. Cleared per capability as G4+ lands. | 05 |
