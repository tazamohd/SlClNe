...
| --- | --- | --- |
| Registered capabilities | 424 | `STATUS.json` |
| Rendering | 424 of 424 | `STATUS.json` |
| Wired to the API | 99 of 424 | `STATUS.json` |
| Reading design fixtures | 285 of 424 | `STATUS.json` |
| End-to-end covered | 424 of 424 | `STATUS.json` |
...
**285 of 424 screens still read design fixtures rather than the API.** Everything else on this dashboard looks healthier than the project is, because "renders and is asserted" is a genuine achievement that is not the same as "works against the server". Read every other row against that one.
...
| BLK-004 | CRITICAL | 285 rendered capabilities are mock-only | They render, but read fixtures rather than an API. Cleared per capability as G4+ lands. | 05 |
