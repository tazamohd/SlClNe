...
`server/src/registry.ts` describes each of the 58 collections once. `server/src/routes/collections.ts` generates 294 endpoints from those descriptions — list, export, detail, and for the 24 writable ones create, update, delete, bulk-update and bulk-delete.
