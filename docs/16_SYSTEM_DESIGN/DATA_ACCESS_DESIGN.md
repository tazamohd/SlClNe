...
`server/src/registry.ts` describes each of the 55 collections once. `server/src/routes/collections.ts` generates 265 endpoints from those descriptions — list, export, detail, and for the 20 writable ones create, update, delete, bulk-update and bulk-delete.
...
`DELETE` sets `deleted_at`; the row stays. 65 tables carry the column and the generic router filters on it. A hard delete is not exposed through the API at all.
