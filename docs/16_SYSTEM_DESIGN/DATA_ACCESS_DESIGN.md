...
`server/src/registry.ts` describes each of the 63 collections once. `server/src/routes/collections.ts` generates 334 endpoints from those descriptions — list, export, detail, and for the 29 writable ones create, update, delete, bulk-update and bulk-delete.
...
`DELETE` sets `deleted_at`; the row stays. 74 tables carry the column and the generic router filters on it. A hard delete is not exposed through the API at all.
