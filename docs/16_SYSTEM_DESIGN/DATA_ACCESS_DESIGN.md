...
`DELETE` sets `deleted_at`; the row stays. 63 tables carry the column and the generic router filters on it. A hard delete is not exposed through the API at all.
