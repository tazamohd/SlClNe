...
| Measure | Count | Of |
| --- | --- | --- |
| Screens reading the live API | 119 | 430 |
| Screens reading design fixtures | 248 | 430 |
| Rendering | 430 | 430 |
...
| --- | --- | --- |
| Loading | 163 of 430 | A screen that renders empty while fetching reads as "no data" and is indistinguishable from a real empty result |
| Error | 130 of 430 | A failed fetch with no error state is a blank screen the user cannot act on |
| Empty | 185 of 430 | Zero rows is a normal state and needs its own design, not a table with no rows |
