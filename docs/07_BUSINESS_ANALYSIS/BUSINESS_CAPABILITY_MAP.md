...
All 430 registered screens and all 449 endpoints map to exactly one capability. That is a property this generator checks, not a claim: an unmapped screen or endpoint is a failure in `docs:check`.
...
flowchart LR
  ...
  CAP_ACCOUNTING["Accounting and finance<br/>8 screens · 59 endpoints"]
  OBJ_CASH --> CAP_ACCOUNTING
  CAP_HR["HR and payroll<br/>5 screens · 52 endpoints"]
  OBJ_CAPACITY --> CAP_HR
  CAP_CRM["CRM and sales<br/>12 screens · 51 endpoints"]
  ...
  CAP_PLATFORM["Administration and platform<br/>35 screens · 28 endpoints"]
  OBJ_CONTROL --> CAP_PLATFORM
  CAP_IDENTITY["Identity and access<br/>19 screens · 24 endpoints"]
  ...
  CAP_DESIGNSYSTEM["Design system and reference surfaces<br/>232 screens · 0 endpoints"]
...
| CAP-ACCOUNTING | Accounting and finance | OBJ-CASH | `accounting` | 8 | 8 | 59 | 10 | 5 |
| CAP-HR | HR and payroll | OBJ-CAPACITY | `hr`, `technicians` | 5 | 5 | 52 | 6 | 10 |
| CAP-CRM | CRM and sales | OBJ-RETENTION | `crm`, `callcenter` | 12 | 10 | 51 | 6 | 7 |
...
| CAP-PLATFORM | Administration and platform | OBJ-CONTROL | `admin`, `settings`, `superadmin`, `dashboard`, `network`, `ungated`, `platform` | 35 | 7 | 28 | 5 | 14 |
...
145 of 430 screens are wired to the live API; the remainder render from the ported design fixtures. That is the single largest fact about the product's current state, and it is measured in `project-control/STATUS.json` rather than asserted here.
...
| Screens | 35 (7 data-backed) |
| Endpoints | 28 |
| Entities | `departments`, `branches`, `integrations`, `oemTools`, `notifications` |
