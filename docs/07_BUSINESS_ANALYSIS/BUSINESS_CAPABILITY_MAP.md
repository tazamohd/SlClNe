...
All 433 registered screens and all 385 endpoints map to exactly one capability. That is a property this generator checks, not a claim: an unmapped screen or endpoint is a failure in `docs:check`.
...
flowchart LR
  ...
  CAP_WORKSHOP["Workshop operations<br/>23 screens · 88 endpoints"]
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| CAP-WORKSHOP | Workshop operations | OBJ-THROUGHPUT | `jobcards`, `appointments`, `estimates` | 23 | 22 | 88 | 14 | 12 |
...
115 of 433 screens are wired to the live API; the remainder render from the ported design fixtures. That is the single largest fact about the product's current state, and it is measured in `project-control/STATUS.json` rather than asserted here.
