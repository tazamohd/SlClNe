...
All 436 registered screens and all 524 endpoints map to exactly one capability. That is a property this generator checks, not a claim: an unmapped screen or endpoint is a failure in `docs:check`.
...
flowchart LR
  ...
  CAP_HR["HR and payroll<br/>12 screens · 70 endpoints"]
  OBJ_CAPACITY --> CAP_HR
  ...
  CAP_DESIGNSYSTEM["Design system and reference surfaces<br/>202 screens · 0 endpoints"]
...
| CAP-HR | HR and payroll | OBJ-CAPACITY | `hr`, `technicians` | 12 | 12 | 70 | 8 | 10 |
...
| CAP-DESIGNSYSTEM | Design system and reference surfaces | OBJ-VISIBILITY | _(screen domain: ui, featuremap)_ | 202 | 36 | 0 | 0 | 0 |
...
162 of 436 screens are wired to the live API; the remainder render from the ported design fixtures. That is the single largest fact about the product's current state, and it is measured in `project-control/STATUS.json` rather than asserted here.
