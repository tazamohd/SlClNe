...
Derived from `project-control/CAPABILITY_REGISTRY.json` (18 capabilities, 424 screens mapped, 303 endpoints mapped), `project-control/PERMISSION_REGISTRY.json` (15 roles, 28 permission modules), `server/src/registry.ts` and the route files.
...
| Supporting components | `server/src/routes/workshop.ts`, `routes/estimates.ts`, `routes/estimate-otp.ts`, `routes/obd.ts`, `routes/workshop-reports.ts`; 16 screens; 61 endpoints |
...
| Supporting components | `routes/collections.ts` (generated customer and fleet routes), `routes/fleets.ts`; 3 screens; 15 endpoints; entities `customers`, `fleets` |
...
| Supporting components | generated vehicle routes in `routes/collections.ts`; 4 screens; 7 endpoints |
| Dependencies | SVC-101, SVC-102, SVC-103, SVC-104, SVC-111 |
...
| Supporting components | `routes/inventory.ts`; 7 screens; 11 endpoints; business rules `BR-INVENTORY-*` in `packages/contract` |
...
| Supporting components | `routes/procurement.ts`, `security/approvals.ts`, `security/sod.ts`; 1 screen; 24 endpoints |
...
| Supporting components | `routes/invoices.ts`; 6 screens; 20 endpoints; money held as integer halalas (ADR-006) |
...
| Supporting components | `routes/bank.ts`, `routes/finance-reports.ts`, `routes/insurance-claims.ts`; 7 screens; 40 endpoints |
...
| Supporting components | `routes/payroll.ts`, `routes/leave.ts`; 5 screens; 41 endpoints |
...
| Supporting components | `routes/crm.ts`, `routes/public.ts`; 12 screens; 35 endpoints |
...
| Supporting components | `routes/workshop-reports.ts`, `routes/product-reports.ts`, `routes/finance-reports.ts`; 11 screens; the registry attributes 0 endpoints to the capability itself because the report endpoints are counted under the capability whose data they read |
...
| Supporting components | `routes/approvals.ts`, `security/approvals.ts`, `security/sod.ts`; ceilings in `PERMISSION_REGISTRY.roleMeta`; six SoD pairs in `PERMISSION_REGISTRY.segregationOfDuties` |
...
| Supporting components | 11 screens across the `portal`, `kiosk` and `call-center` surfaces in `project-control/STATUS.json` |
...
| Supporting components | 10 screens; 6 endpoints; entities `aiAgents`, `conversations` |
...
| Supporting components | 36 screens; 15 endpoints; `GET /api/v1/diagnostics/integrations` reports which integrations are live |
| Dependencies | SVC-101, SVC-102, SVC-103, SVC-104, SVC-111 |
...
| Supporting components | `server/src/auth/*`; 18 screens; 24 endpoints |
...
| Supporting components | 31 public screens; `routes/public.ts` with its own tighter per-IP rate limit (`PUBLIC_LEAD_RATE_LIMIT`) |
...
| Consumers | customer |
| Supporting components | 11 screens; `@capacitor/*` in `app/package.json`; `cap:sync`, `cap:android`, `cap:ios` scripts |
...
| Supporting components | 233 registered screens, 24 data-backed; `check-tokens` gate |
...
| SVC-103 | Tenant and branch isolation — PostgreSQL row-level security keyed on transaction-local context | CURRENT | `server/drizzle/0001_rls.sql`; 64 tables with RLS, all FORCE; see `docs/19_SECURITY/TENANT_ISOLATION.md`. The application connects as a non-superuser role created by `server/scripts/migrate.ts` | all services | Vital |
