...
Derived from `project-control/CAPABILITY_REGISTRY.json` (18 capabilities, every registered screen and every endpoint mapped — the totals are in `project-control/CAPABILITY_REGISTRY.json`, and `node tools/docs/check.mjs` fails if any screen or endpoint maps to no capability), `project-control/PERMISSION_REGISTRY.json` (15 roles, 28 permission modules), `server/src/registry.ts` and the route files.
...
| SVC-110 | Static application delivery — built SPA served with the full security-header set | CURRENT | `Dockerfile` plus `nginx.conf`, `vercel.json`, `.github/workflows/deploy-pages.yml`, `.github/workflows/deploy-hostinger.yml`; headers defined once in `app/security-headers.mjs` and drift-checked by `npm run check-headers` | every browser and mobile consumer | Vital |
