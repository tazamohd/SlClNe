...
|---|---|---|---|---|
| BLK-004 | CRITICAL | 43 rendered capabilities are mock-only | 05 | W2 |
| BLK-013 | HIGH | 127 rendered capabilities have no backend collection yet | 05 | W3 |
...
| NO_BACKEND | 127 | shows the honest gap state: no collection exists yet for it to read |
| CONTENT_ONLY | 67 | auth or public surface: renders content, reads no collection by design |
| MOCK_ONLY | 43 | renders, but from fixtures rather than an API |
